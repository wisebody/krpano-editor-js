...
function onready() {
  ...
  // 기존 핫스팟 선택 이벤트 덮어쓰기, 핫스팟 클릭 이동 이벤트 추가
  krpano
    .get('hotspot')
    .getArray()
    .forEach(function (oldHotSpot) {
      if (oldHotSpot.name !== 'vr_cursor' && oldHotSpot.name !== 'webvr_prev_scene' && oldHotSpot.name !== 'webvr_next_scene') {
...
function changeScene(index) {
  ...
  if (currentScene.hotSpots) {
    krpano
      .get('hotspot')
      .getArray()
      .forEach(function (everySpot) {
        if (everySpot.name !== 'vr_cursor' && everySpot.name !== 'webvr_prev_scene' && everySpot.name !== 'webvr_next_scene') {
...
function updatePbLine() {
  ...
  $('.number-pb-shown')
    .css('left', startPx.toFixed(0) + 'px')
    .css('width', widthPx.toFixed(0) + 'px')
    .show();
}
